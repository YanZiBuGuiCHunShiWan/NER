...
class BertCRFTrainer(BaseTrainer):
    ...
    def _build_model(self):
        """构建bert-crf模型"""
        self.model = BertCRF(self.pretrained_model_dir, self.vocab.label_size)

        # 设置AdamW优化器
        no_decay = ["bias", "LayerNorm.weight"]  # bias和LayerNorm不使用正则化
        # # 区分bert层的参数和crf层的参数,crf层的学习率要大，bert的学习率要小，通常大100倍
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in crf_parameters], 'lr': self.learning_rate * 50}  # # crf层使用较大的学习率
        ]
    ...
    def _transform_batch(self, batch_texts, batch_labels, max_length=512):
        ...
        batch_input_ids = torch.cat(batch_input_ids) #将列表变成张量
        batch_att_mask = torch.cat(batch_att_mask) #将列表变成张量
        batch_label_ids = torch.LongTensor(batch_label_ids) #将列表变成张量
        ...
        for epoch in range(epoch):
            for batch_idx in range(math.ceil(len(train_texts) / batch_size)):
                ...
                if avg_loss and avg_loss < best_loss:
                    best_loss = avg_loss
                    torch.save(self.model.state_dict(), '{}/{}'.format(self.model_dir, self.ckpt_name)) #保存模型
    ...
    def _get_acc_one_step(self, labels_predict_batch, labels_batch):
        ...
        for labels_predict, labels in zip(labels_predict_batch, labels_batch):
            ...
            correct += (active_labels_predict == active_labels.cpu()).sum().item()
