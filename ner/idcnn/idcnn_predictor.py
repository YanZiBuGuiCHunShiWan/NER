...
class IDCNNPredictor(BasePredictor):
    def __init__(self, model_dir, vocab_name='vocab.json'):
        ...
        self._load_config()
        self.vocab.loadvocab('{}/{}'.format(model_dir, vocab_name))
        self._load_model()
    ...
    def _load_model(self):
        """根据config和vocab，加载模型"""
        self.model = IDCNNCRF(
            seqlen=self._config['max_len'], filters=self._config['filters'], hidden_num=self._config['hidden_num'],
            vocab_size=self._config['vocab_size'], embedd_dim=self._config['embedding_size'],
        ...
        for batch_idx in range(math.ceil(len(texts) / batch_size)):  # 分batch
            ...
            with torch.no_grad():
                ...
                for predict_labels, input_len in zip(best_path, input_lens):
                    ...
                    batch_labels.append(
                        [self.vocab.id2tag[str(label_id)] for label_id in predict_labels[:input_len].tolist()]
                    )
